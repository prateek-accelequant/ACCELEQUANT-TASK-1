...
Description: Memory-efficient unified master orchestration engine. Features dynamic runtime 
             worker allocation, central cache organization, positive control integration, 
             live-updating matplotlib figures, and persistent compute-year tracking.
"""
import os
import sys
...
# =====================================================================
COMPUTE_STATE_FILE = os.path.join("results", "compute_footprint_state.json")
...
def update_live_plot(fig, axes, summary_data, dataset_name):
    axes[0].clear()
    axes[1].clear()
    ...
    axes[0].set_title(f"{dataset_name}: Live F1-Score vs N", fontsize=11, fontweight='bold')
    axes[0].set_xlabel("Sample Budget Size (N)")
    axes[0].set_ylabel("F1 Score")
    axes[0].grid(True, linestyle='--', alpha=0.5)
    axes[0].legend()
    
    axes[1].set_title(f"{dataset_name}: Live ROC-AUC vs N", fontsize=11, fontweight='bold')
    ...
    plt.suptitle(f"Live Incremental Performance Feed: {dataset_name}", fontsize=13, fontweight='bold', y=1.02)
    plt.tight_layout()
    plt.ion()
...
def run_central_experiment_incremental(X_all, y_all, dataset_name):
    print(f"\n==================================================================")
    print(f"STARTING MASTER SAMPLE-EFFICIENCY RUN FOR: {dataset_name} (Samples: {len(X_all)})")
    print(f"==================================================================")
    
    estimated_seconds_per_split = 12.5 if len(X_all) <= 500 else 45.0
    total_theoretical_seconds = len(config.N_LIST) * config.N_SPLITS * estimated_seconds_per_split
    print(f"   [Theoretical Time Feedback] Estimated completion time for active sweep: ~{total_theoretical_seconds/60:.1f} minutes.")
    ...
    for q_name, map_type in [('Quantum-ZZ', 'ZZ'), ('Quantum-CPMap', 'CPMap')]:
        if config.RUN_MODELS[q_name]:
            ...
            if os.path.exists(train_cache_file) and os.path.exists(test_cache_file):
                ...
                print(f"   [Cache Miss] Computing {q_name} kernel matrices from scratch in chunks...")
    ...
    for N in config.N_LIST:
        ...
        for split in split_iterator:
            ...
            if 'RBF-SVC' in active_models:
                split_iterator.set_postfix_str("Tuning: RBF-SVC")
            ...
            if 'XGBoost' in active_models:
                split_iterator.set_postfix_str("Tuning: XGBoost")
        ...
        update_live_plot(fig, axes, summary_data, dataset_name)
        gc.collect()
    ...
    cumulative_cpu_years = (total_cumulative_seconds * cpu_cores) / seconds_per_year
    cumulative_gpu_years = (total_cumulative_seconds * 1) / seconds_per_year

    report_str = f"\n{'='*66}\nSAMPLE-EFFICIENCY METRICS REPORT FOR: {dataset_name}\n{'='*66}\n"
    report_str += f"[Persistent Compute Footprint Tracker]\n"
    report_str += f" -> Cumulative Active Wall-Clock Time: {total_cumulative_seconds:.2f} seconds\n"
    report_str += f" -> Cumulative CPU Computing Years: {cumulative_cpu_years:.2e} years\n"
    report_str += f" -> Cumulative GPU Computing Years: {cumulative_gpu_years:.2e} years\n\n"
...
def run_single_ablation(fmap_type, ent, bw, use_noise, X_train_preprocessed, X_test_preprocessed, y_train, y_test, clean_ds):
    """Isolated worker process for parallel structural ablations with memory cleanup and chunking."""
...
def run_ablation_matrix_suite(dataset_name, X_all, y_all):
    """Executes structural ablations using bounded workers."""
    ...
    tasks = [(fmap, ent, bw, noise) for fmap in feature_maps for ent in entanglements for bw in bandwidths for noise in noise_options]
    ...
    with open(txt_out, "w") as f:
        ...
        f.write(f" -> Cumulative Compute Footprint: CPU Years = {cumulative_cpu_years:.2e} | GPU Years = {cumulative_gpu_years:.2e}\n\n")
    ...
    plt.suptitle(f"Incremental Ablation Analytics: {dataset_name}", fontsize=13, fontweight='bold', y=1.02)
    plt.tight_layout()
    ...
    print(f"\n[Ablation Artifact Hub] Results recorded:")
    print(f" -> Cumulative CPU Computing Years: {cumulative_cpu_years:.2e} years")
    print(f" -> Cumulative GPU Computing Years: {cumulative_gpu_years:.2e} years")
    print(f" -> Incremental Plot: {plot_out}\n")
    return df_results


if __name__ == "__main__":
    print("[Master Orchestration Engine Initialization] Preloading distributions...")
    ...
    print("   [Pipeline Setup] Generating positive control datasets...")
    ...
    for name, X_d, y_d in datasets_to_run:
        if X_d is not None:
            ...
            if os.path.exists(sample_eff_txt) and os.path.exists(sample_eff_plot):
                ...
                run_central_experiment_incremental(X_d, y_d, name)
            ...
            if os.path.exists(ablation_csv) and os.path.exists(ablation_txt) and os.path.exists(ablation_plot):
                ...
                run_ablation_matrix_suite(name, X_d, y_d)

    print("\n[Execution Hub Complete] All tasks, tracking metrics, positive controls, and live plots successfully finalized.")