"""
Module: synthetic_engine.py
Description: Dedicated orchestration engine for Synthetic and Quantum-Native datasets.
             Executes both the Main Sample-Efficiency Suite and the Structural Ablation Suite.
"""
import os
import sys
...
# =====================================================================
print("\n" + "="*60)
print("   Quantum Kernel Evaluation - SYNTHETIC DATA ENGINE   ")
print("="*60)
try:
...
# Force Matplotlib to run in headless background mode to prevent silent crashes
import matplotlib
matplotlib.use('Agg')
...
# Force Qiskit Python Multiprocessing to 1 to prevent internal fallbacks
...
# =====================================================================
COMPUTE_STATE_FILE = os.path.join("results", "compute_footprint_synthetic.json")
...
def evaluate_kernel_in_chunks(qkernel, X1, X2=None, chunk_size=200):
    symmetric = False
    if X2 is None:
        X2 = X1
        symmetric = True
...
def run_sample_efficiency_suite(X_all, y_all, dataset_name):
    print(f"\n==================================================================")
    print(f"STARTING SAMPLE-EFFICIENCY SUITE FOR: {dataset_name} (Samples: {len(X_all)})")
    print(f"==================================================================")
    ...
    for q_name, map_type in [('Quantum-ZZ', 'ZZ'), ('Quantum-CPMap', 'CPMap')]:
        if config.RUN_MODELS[q_name]:
            ...
            if os.path.exists(train_cache_file) and os.path.exists(test_cache_file):
                ...
                print(f"   [Cache Miss] Computing {q_name} kernel matrices for {dataset_name}...")
                K_train_master = evaluate_kernel_in_chunks(mgr.kernel, X1=X_train_full, chunk_size=200)
                K_test_master = evaluate_kernel_in_chunks(mgr.kernel, X1=X_test, X2=X_train_full, chunk_size=200)
...
def run_single_ablation(fmap_type, ent, bw, use_noise, X_train_preprocessed, X_test_preprocessed, y_train, y_test, clean_ds):
    ...
    if os.path.exists(train_cache_file) and os.path.exists(test_cache_file):
        ...
        K_train = evaluate_kernel_in_chunks(mgr.kernel, X1=X_train_scaled, chunk_size=200)
        K_test = evaluate_kernel_in_chunks(mgr.kernel, X1=X_test_scaled, X2=X_train_scaled, chunk_size=200)
...
if __name__ == "__main__":
    print("[Synthetic Engine] Preloading data distributions...")
    ...
    for name, X_d, y_d in datasets_to_run:
        ...
        if os.path.exists(sample_eff_txt) and os.path.exists(sample_eff_plot):
            ...
            run_sample_efficiency_suite(X_d, y_d, name)
    ...
    print("\n[Execution Hub Complete] Synthetic Data Pipeline successfully finalized.")